...
fn allocator_assigns_unique_ips() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ip1 = s1.lines().find(|l| l.trim_start().starts_with("ip:")).unwrap().split(':').nth(1).unwrap().trim();
    let ip2 = s2.lines().find(|l| l.trim_start().starts_with("ip:")).unwrap().split(':').nth(1).unwrap().trim();
    let ip3 = s3.lines().find(|l| l.trim_start().starts_with("ip:")).unwrap().split(':').nth(1).unwrap().trim();
