...
mod extra_tests {
    use super::*;
    use crate::{ObuArgs, ObuParameters};
    use indexmap::IndexMap;
...
impl Routing {
    ...
    pub fn get_route_to(&self, mac: Option<MacAddress>) -> Option<Route> {
        ...
        if !self.routes.contains_key(&target_mac) {
            ...
            use node_lib::control::routing_utils::{pick_best_next_hop, NextHopStats};
        ...
        if !latency_candidates.is_empty() {
            ...
            let (best_mac, best_avg) =
                node_lib::control::routing_utils::pick_best_from_latency_candidates(
