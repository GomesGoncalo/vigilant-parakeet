use node_lib::test_helpers::util::{
    await_condition_with_time_advance, mk_device_from_fd, mk_shim_pair,
};
use obu_lib::Obu;
use rsu_lib::Rsu;
mod common;
...
async fn rsu_and_obu_topology_discovery() -> anyhow::Result<()> {
    ...
    let _rsu = Rsu::new(args_rsu, Arc::new(tun_a), Arc::new(dev_a), "test_rsu".to_string())?;
    let obu = Obu::new(args_obu, Arc::new(tun_b), Arc::new(dev_b), "test_obu".to_string())?;
