                ...
                let messages = node::wire_traffic(&device, |pkt, size| {
                    async move {
                        match Message::try_from(&pkt[..size]) {
                            ...
                            Err(e) => {
                                tracing::warn!(error = ?e, raw = %crate::control::node::bytes_to_hex(&pkt[..size]), "rsu wire_traffic failed to parse message");
                                return Ok(None);
                            }
                        }
                    }
        ...
        tokio::task::spawn(async move {
            loop {
                let msg: Vec<Vec<u8>> = {
                    ...
                    tracing::debug!(?msg, "generated hello");
                    (&msg).into()
                };
                ...
                tracing::debug!(n = flat.len(), raw = %crate::control::node::bytes_to_hex(&flat), "rsu generated raw");
                tracing::debug!(n = flat.len(), raw = %crate::control::node::bytes_to_hex(&flat), "rsu hello_task sending bytes");
