...
/// Check if data appears to be encrypted (has minimum length and starts with reasonable nonce).
/// This is a heuristic to avoid double-encryption.
pub fn is_likely_encrypted(data: &[u8]) -> bool {
    // Must be at least 12 bytes (nonce) + 16 bytes (minimum GCM tag)
    data.len() >= 28
}
