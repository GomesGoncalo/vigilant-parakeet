...
impl Hub {
    pub fn new(hub_fds: Vec<i32>, delays_ms: [[u64; 3]; 3]) -> Self {
        Self { hub_fds, delays_ms, watch_up_from_idx: None, watch_flag: None, watch_from_mac: None, watch_to_mac: None }
    }

    pub fn with_upstream_watch(mut self, idx: usize, from: MacAddress, to: MacAddress, flag: Arc<AtomicBool>) -> Self {
        ...
        tokio::spawn(async move {
            ...
            loop {
                for i in 0..hub_fds.len() {
                    ...
                    let n = unsafe { libc::recv(hub_fds[i], buf.as_mut_ptr() as *mut _, buf.len(), 0) };
                    if n > 0 {
                        let n = n as usize;
                        buf.truncate(n);
                        ...
                        if let (Some(idx), Some(flag)) = (watch_idx, watch_flag.as_ref()) {
                            if idx == i {
                                if let Ok(msg) = node_lib::messages::message::Message::try_from(&buf[..]) {
                                    if let node_lib::messages::packet_type::PacketType::Data(node_lib::messages::data::Data::Upstream(_)) = msg.get_packet_type() {
                                        if watch_from.map(|m| msg.from().ok() == Some(m)).unwrap_or(true)
                                            && watch_to.map(|m| msg.to().ok() == Some(m)).unwrap_or(true) {
                        ...
                        for (j, out_fd) in hub_fds.iter().copied().enumerate() {
                            ...
                            let data = buf.clone();
                            tokio::spawn(async move {
                                if delay.as_millis() > 0 { tokio::time::sleep(delay).await; }
                                let _ = unsafe { libc::send(out_fd, data.as_ptr() as *const _, data.len(), 0) };
                            });
                        }
                    }
                }
