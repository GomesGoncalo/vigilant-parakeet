...
Notes:
- The simulator builds nodes using `node_lib` or by launching `node` binaries, configures network namespaces, and collects per-node state (used by visualization).
- `sim_args.rs` contains CLI/experiment wiring.
