...
pub fn render_topology_tab(f: &mut Frame, area: Rect, state: &mut TopologyTabState) {
    ...
    for (name, n) in state.nodes.iter() {
        name_by_mac.insert(n.mac.clone(), name.clone());
    }
    ...
    for (name, n) in state.nodes.iter() {
        ...
        if let crate::simulator::SimNode::Obu(ref o) = n.simnode {
    ...
    for name in unattached.iter() {
        if !parent_map.contains_key(name) && !rsus.contains(name) {
            if let Some(n) = state.nodes.get(name) {
                unattached_display.push(format!("{} ({})", name, n.mac));
            } else {
...
fn render_node_details(
    ...
    while !seen_path.contains(&current) {
        ...
        if let Some(n) = state.nodes.get(&current) {
            if n.node_type == "Rsu" {
                reached_rsu = true;
                break;
            }
...
fn collect_node(
    ...
    let label = if let Some(n) = ctx.state.nodes.get(name) {
        format!("{} ({})", name, n.mac)
    } else {
        name.to_string()
    };
