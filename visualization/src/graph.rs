...
fn bezier_points(x0: f64, y0: f64, x1: f64, y1: f64, offset: f64, samples: usize) -> (Vec<f64>, Vec<f64>) {
    let mx = (x0 + x1) / 2.0;
    let my = (y0 + y1) / 2.0;
    let dx = x1 - x0;
    let dy = y1 - y0;
...
fn pick_node_type(map: &std::collections::HashMap<String, JsonValue>, node: &str) -> Option<String> {
    map.get(node).and_then(|ni| ni.get("node_type").and_then(|v| v.as_str()).map(|s| s.to_string()))
}
...
pub fn graph(props: &GraphProps) -> Html {
    ...
    use_effect_with((nodes.clone(), channels.clone(), node_info.clone(), stats.clone()), move |(_n, _c, _u, _s)| {
        // compute RSUs and OBUs
        let mut rsus = Vec::new();
        let mut obus = Vec::new();
        for n in &nodes {
            if let Some(t) = pick_node_type(&node_info, n) {
                if t.to_lowercase() == "rsu" { rsus.push(n.clone()); continue; }
            }
            obus.push(n.clone());
        }
        ...
        for (i, o) in obus.iter().enumerate() {
            let angle = (2.0*std::f64::consts::PI*(i as f64))/ (if obu_count>0 { obu_count as f64 } else {1.0});
            ...
            if let Some(info) = node_info.get(o) {
                if let Some(up) = info.get("upstream") {
                    if let Some(un) = up.get("node_name").and_then(|v| v.as_str()) {
                        if let (Some((ox, oy)), Some((ux, _uy))) = (positions.get(o).cloned(), positions.get(un).cloned()) {
                            let newx = (ox + ux)/2.0;
                ...
                JsonValue::Object(map) => {
                    for (_k, v) in map {
                        if let JsonValue::Object(o) = v {
                            for (_k2, v2) in o {
                                if let JsonValue::Number(n) = v2 { sum += n.as_f64().unwrap_or(0.0); }
                            }
                        } else if let JsonValue::Number(n) = v { sum += n.as_f64().unwrap_or(0.0); }
                    }
                }
                JsonValue::Array(arr) => {
                    for item in arr {
                        if let JsonValue::Object(o) = item {
                            for (_k,v) in o { if let JsonValue::Number(n)=v { sum += n.as_f64().unwrap_or(0.0); } }
                        }
                    }
                }
                _ => {}
        ...
        // update snapshot for next run
        {
            ...
            for (k,v) in node_sums.into_iter() { ns.insert(k,v); }
    ...
    let mut annotations: Vec<JsonValue> = Vec::new();
        for (from, to) in &edges {
            if let (Some((sx, sy)), Some((tx, ty))) = (positions.get(from).cloned(), positions.get(to).cloned()) {
                // baseline straight faint line (for context)
                let mut baseline = serde_json::Map::new();
                baseline.insert("x".to_string(), JsonValue::Array(vec![JsonValue::Number(serde_json::Number::from_f64(sx).unwrap()), JsonValue::Number(serde_json::Number::from_f64(tx).unwrap())]));
                baseline.insert("y".to_string(), JsonValue::Array(vec![JsonValue::Number(serde_json::Number::from_f64(sy).unwrap()), JsonValue::Number(serde_json::Number::from_f64(ty).unwrap())]));
                baseline.insert("mode".to_string(), JsonValue::String("lines".to_string()));
                baseline.insert("line".to_string(), JsonValue::Object({ let mut m = serde_json::Map::new(); m.insert("color".to_string(), JsonValue::String("#333".to_string())); m.insert("width".to_string(), JsonValue::Number(serde_json::Number::from(1))); m.insert("dash".to_string(), JsonValue::String("dot".to_string())); m }));
                baseline.insert("hoverinfo".to_string(), JsonValue::String("none".to_string()));
                ...
                // choose offsets
                let dx = tx - sx; let dy = ty - sy; let dist = (dx*dx + dy*dy).sqrt().max(1.0);
                ...
                if up_bps > 0.0 {
                    ...
                    trace.insert("x".to_string(), JsonValue::Array(xs.into_iter().map(|v| JsonValue::Number(serde_json::Number::from_f64(v).unwrap())).collect()));
                    trace.insert("y".to_string(), JsonValue::Array(ys.into_iter().map(|v| JsonValue::Number(serde_json::Number::from_f64(v).unwrap())).collect()));
                    trace.insert("mode".to_string(), JsonValue::String("lines".to_string()));
                    trace.insert("hoverinfo".to_string(), JsonValue::String("text".to_string()));
                    trace.insert("text".to_string(), JsonValue::Array(vec![]));
                    trace.insert("line".to_string(), JsonValue::Object({ let mut m = serde_json::Map::new(); m.insert("color".to_string(), JsonValue::String("rgba(200,30,30,0.9)".to_string())); m.insert("width".to_string(), JsonValue::Number(serde_json::Number::from(2))); m }));
                    ...
                    // midpoint marker
                    let mx = (sx + tx)/2.0; let my = (sy + ty)/2.0;
                    let mut mid = serde_json::Map::new();
                    mid.insert("x".to_string(), JsonValue::Array(vec![JsonValue::Number(serde_json::Number::from_f64(mx).unwrap())]));
                    mid.insert("y".to_string(), JsonValue::Array(vec![JsonValue::Number(serde_json::Number::from_f64(my).unwrap())]));
                    mid.insert("mode".to_string(), JsonValue::String("markers".to_string()));
                    mid.insert("type".to_string(), JsonValue::String("scatter".to_string()));
                    mid.insert("marker".to_string(), JsonValue::Object({ let mut mm = serde_json::Map::new(); mm.insert("size".to_string(), JsonValue::Number(serde_json::Number::from(8))); mm.insert("color".to_string(), JsonValue::String("rgba(200,30,30,0.8)".to_string())); mm }));
                    mid.insert("text".to_string(), JsonValue::Array(vec![JsonValue::String(format!("{} → {}\nup: {} bps", from, to, up_bps as i64))]));
                    mid.insert("hoverinfo".to_string(), JsonValue::String("text".to_string()));
                ...
                if down_bps > 0.0 {
                    ...
                    trace.insert("hoverinfo".to_string(), JsonValue::String("text".to_string()));
                    trace.insert("text".to_string(), JsonValue::Array(vec![]));
                    trace.insert("line".to_string(), JsonValue::Object({ let mut m = serde_json::Map::new(); m.insert("color".to_string(), JsonValue::String("rgba(30,100,200,0.9)".to_string())); m.insert("width".to_string(), JsonValue::Number(serde_json::Number::from(2))); m }));
                    trace.insert("showlegend".to_string(), JsonValue::Bool(false));
                    trace.insert("type".to_string(), JsonValue::String("scatter".to_string()));
                    data_traces.push(JsonValue::Object(trace));
                    // midpoint marker
                    ...
                    mid.insert("mode".to_string(), JsonValue::String("markers".to_string()));
                    mid.insert("type".to_string(), JsonValue::String("scatter".to_string()));
                    mid.insert("marker".to_string(), JsonValue::Object({ let mut mm = serde_json::Map::new(); mm.insert("size".to_string(), JsonValue::Number(serde_json::Number::from(8))); mm.insert("color".to_string(), JsonValue::String("rgba(30,100,200,0.8)".to_string())); mm }));
                    mid.insert("text".to_string(), JsonValue::Array(vec![JsonValue::String(format!("{} → {}\ndown: {} bps", from, to, down_bps as i64))]));
                    mid.insert("hoverinfo".to_string(), JsonValue::String("text".to_string()));
                    mid.insert("showlegend".to_string(), JsonValue::Bool(false));
                    data_traces.push(JsonValue::Object(mid));
                }
                ...
                let mut push_ann = |ax: f64, ay: f64, x: f64, y: f64, color: &str, width: f64| {
                    let mut ann = serde_json::Map::new();
                    ann.insert("x".to_string(), JsonValue::Number(serde_json::Number::from_f64(x).unwrap()));
                    ann.insert("y".to_string(), JsonValue::Number(serde_json::Number::from_f64(y).unwrap()));
                    ann.insert("ax".to_string(), JsonValue::Number(serde_json::Number::from_f64(ax).unwrap()));
                    ann.insert("ay".to_string(), JsonValue::Number(serde_json::Number::from_f64(ay).unwrap()));
                    ...
                    ann.insert("arrowhead".to_string(), JsonValue::Number(serde_json::Number::from(3)));
                    ann.insert("arrowsize".to_string(), JsonValue::Number(serde_json::Number::from_f64(1.0).unwrap()));
                    ann.insert("arrowwidth".to_string(), JsonValue::Number(serde_json::Number::from_f64(width).unwrap()));
                    ann.insert("arrowcolor".to_string(), JsonValue::String(color.to_string()));
                    ann.insert("opacity".to_string(), JsonValue::Number(serde_json::Number::from_f64(0.95).unwrap()));
                    annotations.push(JsonValue::Object(ann));
                };

                // if we drew an up-curve, place arrow along that bezier near destination
                if up_bps > 0.0 {
                    let (xs2, ys2) = bezier_points(sx, sy, tx, ty, dist * offset_scale(up_bps), 30);
                    if xs2.len() >= 3 {
                        let last = xs2.len() - 1;
                ...
                if down_bps > 0.0 {
                    let (xs2, ys2) = bezier_points(sx, sy, tx, ty, -dist * offset_scale(down_bps), 30);
                    if xs2.len() >= 3 {
                        let last = xs2.len() - 1;
                        let ax = xs2[last.saturating_sub(2)];
                        let ay = ys2[last.saturating_sub(2)];
            ...
            // add node markers (draw on top of traces)
            {
                ...
                for (name, (x,y)) in &positions {
                    ...
                    let label = if let Some(nt) = pick_node_type(&node_info, name) { format!("{} ({})", name, nt) } else { name.clone() };
                ...
                node_trace.insert("mode".to_string(), JsonValue::String("markers+text".to_string()));
                node_trace.insert("text".to_string(), JsonValue::Array(texts));
                node_trace.insert("textposition".to_string(), JsonValue::String("top center".to_string()));
                node_trace.insert("hoverinfo".to_string(), JsonValue::String("text".to_string()));
                ...
                node_trace.insert("marker".to_string(), JsonValue::Object({ let mut m = serde_json::Map::new(); m.insert("color".to_string(), JsonValue::Array(colors)); m.insert("size".to_string(), JsonValue::Array(sizes)); m }));
            ...
            {
                let tag = "[VP-GRAPH]";
                let pos_json = serde_json::to_string(&positions).unwrap_or_else(|_| "<pos-serde-err>".to_string());
                console::log_1(&JsValue::from_str(&format!("{} positions: {}", tag, pos_json)));
                let bps_json = serde_json::to_string(&node_bps).unwrap_or_else(|_| "<bps-serde-err>".to_string());
                console::log_1(&JsValue::from_str(&format!("{} node_bps: {}", tag, bps_json)));
                console::log_1(&JsValue::from_str(&format!("{} data_traces_count: {}", tag, data_traces.len())));
                let types: Vec<String> = data_traces.iter().take(8).map(|t| match t {
                    JsonValue::Object(m) => m.get("type").and_then(|v| v.as_str()).unwrap_or("obj").to_string(),
                    _ => "other".to_string()
                }).collect();
                console::log_1(&JsValue::from_str(&format!("{} trace_types: {:?}", tag, types)));
            }
        ...
        layout_map.insert("xaxis".to_string(), JsonValue::Object({ let mut mm = serde_json::Map::new(); mm.insert("showgrid".to_string(), JsonValue::Bool(false)); mm.insert("zeroline".to_string(), JsonValue::Bool(false)); mm.insert("showticklabels".to_string(), JsonValue::Bool(false)); mm.insert("fixedrange".to_string(), JsonValue::Bool(true)); mm }));
        layout_map.insert("yaxis".to_string(), JsonValue::Object({ let mut mm = serde_json::Map::new(); mm.insert("showgrid".to_string(), JsonValue::Bool(false)); mm.insert("zeroline".to_string(), JsonValue::Bool(false)); mm.insert("showticklabels".to_string(), JsonValue::Bool(false)); mm.insert("fixedrange".to_string(), JsonValue::Bool(true)); mm }));
        layout_map.insert("margin".to_string(), JsonValue::Object({ let mut mm = serde_json::Map::new(); mm.insert("l".to_string(), JsonValue::Number(0.into())); mm.insert("r".to_string(), JsonValue::Number(0.into())); mm.insert("t".to_string(), JsonValue::Number(0.into())); mm.insert("b".to_string(), JsonValue::Number(0.into())); mm }));
        ...
        if let Some(win) = window() {
            ...
            let mut ni_map: std::collections::BTreeMap<String, JsonValue> = std::collections::BTreeMap::new();
            for (k,v) in &node_info { ni_map.insert(k.clone(), v.clone()); }
            ...
            for (from, to) in &edges {
                let src = if from.is_empty() { continue; } else { from.clone() };
                let tgt = if to.is_empty() { continue; } else { to.clone() };
            ...
            for nval in nodes_arr.iter_mut() {
                if let JsonValue::Object(map) = nval {
                    ...
                    if !map.contains_key("id") {
                        if let Some(JsonValue::String(lbl)) = map.get("label") { map.insert("id".to_string(), JsonValue::String(lbl.clone())); }
                    }
                    // ensure position object exists and has numeric x/y
                    let mut px = None;
                    let mut py = None;
                    if let Some(JsonValue::Number(nx)) = map.get("x") { px = nx.as_f64(); }
                    if let Some(JsonValue::Number(ny)) = map.get("y") { py = ny.as_f64(); }
                    if let Some(JsonValue::Object(pos)) = map.get("position") { // check nested position
                        if let Some(JsonValue::Number(nx)) = pos.get("x") { px = px.or(nx.as_f64()); }
                        if let Some(JsonValue::Number(ny)) = pos.get("y") { py = py.or(ny.as_f64()); }
                    }
                    let xval = px.unwrap_or(0.0);
                    let yval = py.unwrap_or(0.0);
                    let mut posmap = serde_json::Map::new();
                    posmap.insert("x".to_string(), JsonValue::Number(serde_json::Number::from_f64(xval).unwrap()));
                    posmap.insert("y".to_string(), JsonValue::Number(serde_json::Number::from_f64(yval).unwrap()));
                    map.insert("position".to_string(), JsonValue::Object(posmap));
                    // also keep top-level x/y as numbers
                    map.insert("x".to_string(), JsonValue::Number(serde_json::Number::from_f64(xval).unwrap()));
                    map.insert("y".to_string(), JsonValue::Number(serde_json::Number::from_f64(yval).unwrap()));
                }
            }

            // debug: serialize nodes/edges to console so the browser shows exact payload
            {
                let _ = console::log_1(&JsValue::from_str(&format!("[VP-GRAPH] nodes={} edges={}", nodes_arr.len(), edges_arr.len())));
            }
